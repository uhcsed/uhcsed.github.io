...
const SelectName = styled.span<{ filtered: boolean }>`
  ...
  ${FontVariant.body_sm}
  margin-right: 6px;
  color: ${props => (props.filtered ? Color.red : Color.gray700)};
  white-space: nowrap;
`
...
const Value = styled.span<{ filtered: boolean }>`
  display: flex;
  overflow: hidden;
  border: 1px solid ${props => (props.filtered ? Color.red : Color.gray300)};
  ...
  &:hover {
    color: ${props => (props.filtered ? Color.red : Color.black)};
  }
`
      ...
      <SelectBody onClick={toggleList}>
        <Value filtered={isOptionExist(optionSelected)} onBlur={onBlur} tabIndex={0}>
          ...
          {optionOpen ? (
            <IoChevronUpOutline
              size={20}
              color={`${isOptionExist(optionSelected) ? Color.red : Color.gray700}`}
            />
          ) : (
